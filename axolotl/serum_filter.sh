...
./lofreq_gt.sh -i `dirname $vcf`/COSMO_`basename $vcf` -g 1/1 -n 20 -o `dirname $vcf`/COSMOS_`basename $vcf`
...
#bcftools filter
for vcf in ./gatk*bcftools*vcf.gz
do
tabix -f $vcf
#filtering for DP and QUAL
bcftools filter -O z -o `dirname $vcf`/filter_`basename $vcf` -i 'INFO/DP>0 & QUAL>=0' $vcf
